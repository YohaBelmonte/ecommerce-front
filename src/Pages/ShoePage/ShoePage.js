...
//Funciones para la visualizacion de cada item
const { product } = useHome();
  const IdproductShoe = localStorage.getItem("selectShoe") || "";


const productShoe = product.map((item) => {
  if (item._id == IdproductShoe) {
    return item
  }
})
console.log(productShoe);


//Funcion para mover imagenes de zapas
  ...
  return (
    <div>
      {product?.map((item, i) => {
        return (
        <div className="card-wrapperShoe" key={i}>
